...
它不看自述，只读 Codex、Claude Code、OpenCode、OpenClaw、Cursor、VS Code 的真实运行卷宗，把你这一段时间的 `vibecoding` 轨迹炼成一张修仙卡。
...
做了个有点中二但很实用的东西：`修仙.skill`

它会读你真实的 agent 卷宗，蒸馏你最近的 `vibecoding` 能力，再给你一张修仙卡。
...
最近把一个一直想做的 idea 做出来了，叫 `修仙.skill`。

我越来越觉得，大家和 AI 协作的关键差距，是“已经修到了哪一层”。会不会用只是起点。有人还停在单轮问答，有人已经能把 workflow、tools、multi-agent orchestration 跑成稳定法门。

所以我做了个 skill，直接去读真实卷宗，不靠自述，把一段时间里的协作轨迹炼成一张修仙卡。上面会写你当前的境界、L1-L10 等级、能力判词、破境之法，还会带上 token、样本规模、模型信息这些真实依据。
